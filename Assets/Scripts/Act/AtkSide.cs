using UnityEngine;
/// <summary>
/// 가까운 유닛을 공격하는 행동 (좌우 탐색)
/// </summary>
[CreateAssetMenu(fileName = "AtkASide", menuName = "Action/Atk A Side")]
public class AtkASideEnemy : ActBase
{
    public float atkX = 1; // 공격 배율
    public int atkSize = 1; // 공격 크기 (2이상은 공격시 해당방향으로 관통 공격)
    public int atkCount = 1; // 공격 횟수
    public int Range = 1; // 탐색 가능 거리

    public async override Awaitable Act()
    {
        UnitBase target = Map.Instance.GetClosestUnit(owner, out int closestDist);

        if (target == null || closestDist > Range) return;
        ...
        {
            if (target.isDead) break; // 첫 타겟이 죽으면 공격 중단 (관통 공격이므로)
            ...
            await Awaitable.WaitForSecondsAsync(aniLength * 0.5f); // 공격 타이밍
